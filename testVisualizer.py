...
NUM_BARS = 13
LEDS_PER_BAR = 5
NUM_LEDS = NUM_BARS * LEDS_PER_BAR      # 13 x 5 = 65
DATA_PIN = 28
MACHINE_STATE = 0

# Mode is "GRB" for typical WS2812 strips
...
max_height_update_interval = 900  # ms
...
def update_max_heights():
    ...
    if time.ticks_diff(now, last_max_height_update) > max_height_update_interval:
        ...
        for i in range(NUM_BARS):
            max_heights[i] = random.randint(2, LEDS_PER_BAR)  # 2 to 5 inclusive

def bar_to_hue(bar, num_bars):
    # Spread hues from blue (43690) to pink (54612), fits your library's colorHSV
